...
async def test_mcp_tab_lifecycle_full_e2e() -> None:
    ...
    try:
        ...
        ), f"Expected 1 tab, got {response.data['count']}"
        ...
        ), f"Expected 2 tabs, got {response.data['count']}"
        ...
        response = await browser_navigate_tool(
            manager, action="switch_tab", tab_id=tab1_id
        )
        ...
        response = await browser_navigate_tool(
            manager, action="goto", url="https://example.net"
        )
        ...
        response = await browser_navigate_tool(
            manager, action="close_tab", tab_id=tab2_id
        )
        ...
        assert (
            response.data["count"] == 2
        ), f"Expected 2 tabs after close, got {response.data['count']}"
        ...
        assert (
            response.success
        ), f"list_tabs after second close failed: {response.error}"
        assert response.data is not None
        assert (
            response.data["count"] == 1
        ), f"Expected 1 tab after second close, got {response.data['count']}"
...
async def test_mcp_tab_switch_validation() -> None:
    ...
    try:
        ...
        response = await browser_navigate_tool(
            manager, action="switch_tab", tab_id=None
        )
...
async def test_mcp_tab_antidetect_injection() -> None:
    ...
    try:
        ...
        instance = await manager.get_or_create_instance(
            headless=True, anti_detect=True, use_pool=False
        )
        assert instance.driver is not None
        manager.set_current_instance(instance.id)
        ...
        assert response.data is not None
        tab1_id = [
            t["tab_id"] for t in response.data["tabs"] if t["tab_id"] != tab2_id
        ][0]
        ...
        response = await browser_navigate_tool(
            manager, action="switch_tab", tab_id=tab2_id
        )
        assert response.success
...
async def test_mcp_tab_error_handling() -> None:
    ...
    try:
        ...
        response = await browser_navigate_tool(
            manager, action="switch_tab", tab_id="invalid-tab-id-12345"
        )
        assert not response.success, "Should fail switching to invalid tab"

        # Try to close non-existent tab
        response = await browser_navigate_tool(
            manager, action="close_tab", tab_id="invalid-tab-id-67890"
        )
