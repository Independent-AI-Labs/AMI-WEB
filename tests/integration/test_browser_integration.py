...
def setup_module():
    ...
    loop.run_until_complete(_chrome_manager.initialize())

    loop.close()

    logger.info(f"Browser launched: {_browser.id}")
...
def teardown_module():
    ...
    try:
        # Force terminate browser if it exists
        if _browser and _browser.driver:
            try:
                loop.run_until_complete(_browser.terminate(force=True))
                logger.info("Browser terminated")
            except Exception as e:
                logger.debug(f"Browser termination error (continuing): {e}")

        # Stop chrome manager
        if _chrome_manager:
            try:
                loop.run_until_complete(_chrome_manager.stop())
                logger.info("Chrome manager stopped")
            except Exception as e:
                logger.debug(f"Manager stop error (continuing): {e}")
    finally:
...
class TabContext:
    """Context manager for tab management."""
    ...
    def __enter__(self):
        ...
        self.new_tab = [t for t in tabs if t != self.original_tab][-1]

        # Switch to new tab
        self.browser.driver.switch_to.window(self.new_tab)
        return self.browser
...
class TestBrowserNavigation:
    ...
    async def test_navigate_to_page(self):
        ...
        with TabContext(_browser) as browser:
...
class TestDynamicContent:
    ...
    @pytest.mark.asyncio
    async def test_ajax_content_loading(self):
    ...
    @pytest.mark.asyncio
    async def test_modal_interaction(self):
...
class TestCaptchaHandling:
    ...
    def teardown_class(cls):
        """Clean up any remaining tabs after all CAPTCHA tests."""
        # Close any extra tabs that might have been left open
        if _browser and _browser.driver:
            try:
                handles = _browser.driver.window_handles
                if len(handles) > 1:
                    original = handles[0]
                    for handle in handles[1:]:
                        _browser.driver.switch_to.window(handle)
                        _browser.driver.close()
                    _browser.driver.switch_to.window(original)
            except Exception as e:
                logger.debug(f"Error cleaning up tabs: {e}")

    @pytest.mark.asyncio
    async def test_text_captcha(self):
    ...
    @pytest.mark.asyncio
    async def test_math_captcha(self):
...
class TestBrowserPool:
    ...
    async def test_create_multiple_instances(self):
        ...
        # List instances
        instances = await _chrome_manager.list_instances()
        min_instances = 2
        assert len(instances) >= min_instances

        # Terminate instances
        await _chrome_manager.terminate_instance(instance1.id)
        await _chrome_manager.terminate_instance(instance2.id)
    ...
    async def test_pool_warm_instances(self):
        ...
        # Return to pool
        success = await _chrome_manager.return_to_pool(instance.id)
        assert success is True

        # Check that instance is now available in pool
        stats_after_return = await _chrome_manager.get_pool_stats()
        assert stats_after_return["available"] > 0

        # Get from pool again (should be warm - quick to get)
        instance2 = await _chrome_manager.get_or_create_instance()
        ...
        stats_after_get = await _chrome_manager.get_pool_stats()
    ...
    async def test_parallel_operations(self):
        ...
        try:
            ...
            final_stats = await _chrome_manager.get_pool_stats()
...
class TestScriptInjection:
    ...
    @pytest.mark.asyncio
    async def test_inject_custom_script(self):
    ...
    @pytest.mark.asyncio
    async def test_modify_dom(self):
    ...
    @pytest.mark.asyncio
    async def test_intercept_network_requests(self):
