...
def _parse_json_results(payload: Any, max_results: int) -> list[SearchResult]:
    ...
    for index, item in enumerate(items):
        ...
        results.append({"rank": index + 1, "title": cleaned_title, "url": url, "snippet": cleaned_snippet})
...
def _collect_structured_html_results(
    ...
    results: list[SearchResult] = []
    rank = start_rank
    for container_candidate in cast(Iterable[Any], soup.select("article, div.result, div.result-default, li.result, div.web-result")):
...
async def _fetch_payload(session: ClientSession, url: str, headers: dict[str, str]) -> tuple[str, Any]:
    async with session.get(url, headers=headers) as response:
        ...
        if "json" in content_type:
            ...
            except (ClientError, json.JSONDecodeError) as exc:  # pragma: no cover - defensive
